...
document.getElementById('partyForm').addEventListener('submit', async function(e) {
  ...
  // Send to webhook
  const result = await sendToWebhook(WEBHOOK_URLS.montessori, formData);
...
function validateForm() {
  ...
  // Menu validation - at least one menu item should have count > 0
  const menuCounts = [
    parseInt(document.querySelector('input[name="menu_children_1_count"]')?.value || 0),
    parseInt(document.querySelector('input[name="menu_children_2_count"]')?.value || 0),
    parseInt(document.querySelector('input[name="menu_children_3_count"]')?.value || 0),
  ...
  if (!hasMenuSelection) {
    showErrorMessage('Παρακαλώ επιλέξτε τουλάχιστον ένα μενού με άτομα.');
    isValid = false;
  }
...
function collectFormData() {
  ...
  const extras = {};
  ['clown', 'animatore', 'magos', 'prive_party'].forEach(extra => {
  ...
  const menus = {
    children: {
      menu_1: parseInt(document.querySelector('input[name="menu_children_1_count"]')?.value || 0),
      menu_2: parseInt(document.querySelector('input[name="menu_children_2_count"]')?.value || 0),
      menu_3: parseInt(document.querySelector('input[name="menu_children_3_count"]')?.value || 0)
    },
    adults: {
  ...
  // Generate form ID
  const formId = generateFormId('MONT');
  
  // Build payload
  return {
    form_type: 'montessori_regular',
    form_id: formId,
    ...
    notes: notes,
    signature: signature,
    contactPhone: CONTACT.montessori.phoneLink,
    contactPhoneDisplay: CONTACT.montessori.phone,
