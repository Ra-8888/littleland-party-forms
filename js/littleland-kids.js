...
document.getElementById('partyForm').addEventListener('submit', async function(e) {
  ...
  // Send to webhook
  const result = await sendToWebhook(WEBHOOK_URLS.littleland, formData);
...
function collectFormData() {
  ...
  const extras = {};
  ['clown', 'animatore', 'magos', 'prive1', 'prive2'].forEach(extra => {
  ...
  // Generate form ID
  const formId = generateFormId('LL');
